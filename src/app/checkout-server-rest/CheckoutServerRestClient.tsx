...
import { getBrowserSafeClientId, createOrder, captureOrder } from "@/actions/paypal";

type PaymentStatus = "idle" | "success" | "cancel" | "error";

// No useEligibleMethods() here on purpose: this page tests server-side
// hydration via a hand-rolled REST call (the fetchEligibleMethods server
// action) in isolation, with no client-side eligibility fetch.
const PaymentButtons = ({
  ...
  const { loadingStatus, error: hydrationError } = usePayPal();
  ...
  if (hydrationError) {
    return (
      <div className="py-8 text-center text-sm text-[var(--error)]">
        ...
        {hydrationError.message || "Please refresh the page and try again."}
      </div>
    );
  }
...
const CheckoutServerRestClient = ({
      ...
      <section className="flex-1 flex flex-col items-center px-6 py-16">
        <div className="max-w-lg w-full">
          <h1 className="text-3xl font-semibold tracking-tight text-[var(--foreground)] mb-10">
            Checkout (custom REST, server-only)
          </h1>
...
export default CheckoutServerRestClient;
